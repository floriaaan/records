...
pub trait CollectionUseCase: Send + Sync {
    ...
    async fn get_user_tokens(
        ...
        db: &mut ConnectionDb, 
        user_id: i32
    ) -> Result<Vec<CollectionToken>, AppError>;
...
impl CollectionUseCase for CollectionUseCaseImpl {
    ...
    #[instrument(name = "collection_use_case/get_user_tokens", skip_all)]
    async fn get_user_tokens(
        &self, 
        repos: &Repositories, 
        db: &mut ConnectionDb, 
    ...
    async fn delete_token(
        ...
        let token_opt = repos
            .collection_token
            .find_by_token(&mut **db, &token_id.to_string())
            .await
            .map_err(|e| AppError::from(e))?;
